...
module.exports = function (dep) {
  const getAsync = function (uri) {
    const { log, setting, help, url, querystring, HockeyApp } = dep
    log.log('Using HockeyApp...')

    return new Promise((resolve, reject) => {
      const patataSettings = setting.get()
      const fullUrl = url.parse(uri)
      ...
      if (hockeyAppName === null && hockeyAppId === null) {
        help.errorDueNotHockeyAppName(dep)
      }
      if ((filterName === null || filterName === '') !== (filterValue === null || filterValue === '')) { // !XOR condition
        help.errorDueMalformedHockeyAppFilter(dep)
      }
